...
public class ExtractService
{
  ...
  public static void launchGenerateExtract(ExtractItem extractItem)
  {
    ...
    String znode =
        ExtractManager.getControlDir() + File.separator + "launchTarget-" + extractItem.getExtractName() + "-"
  ...
  private static boolean getZKConnection()
  {
    if (!isConnectionValid(zk))
    {
      ...
      do
      {
        try
        {
          zk = new ZooKeeper(Deployment.getZookeeperConnect(), 3000, new Watcher()
          {
          ...
          if (!isConnectionValid(zk))
          {
            log.info("Could not get a zookeeper connection, waiting... (" + (NB_TIMES_TO_TRY - nbLoop)
                + " more times to go)");
            try
            {
              Thread.sleep(5 * 1000);

...
public class MetricHistory
{
  ...
  ****************************************/
  ...
  public synchronized void update(Date date, long value, Object briefcase)
  {
    ...
    ****************************************/
    ...
    Date day = RLMDateUtils.truncate(effectiveDate, Calendar.DATE, Deployment.getDeployment(tenantID).getTimeZone());
  ...
  *****************************************/
  ...
  public synchronized Long getValue(Date startDay, Date endDay) throws IllegalArgumentException
  {
    ...
    ****************************************/
    ...
    if (startDay != null && ! Objects.equals(startDay, RLMDateUtils.truncate(startDay, Calendar.DATE, Deployment.getDeployment(tenantID).getTimeZone())))
      {
    ...
    if (endDay != null && ! Objects.equals(endDay, RLMDateUtils.truncate(endDay, Calendar.DATE, Deployment.getDeployment(tenantID).getTimeZone())))
      {
  ...
  ****************************************/
  ...
  {
    Date day = RLMDateUtils.truncate(evaluationDate, Calendar.DAY_OF_WEEK, Deployment.getDeployment(tenantID).getTimeZone());
    Date startDay = day;
    Date endDay = RLMDateUtils.addDays(day, 7, Deployment.getDeployment(tenantID).getTimeZone());
    return this.getValue(startDay, endDay);
  }
  ...
  {
    Date day = RLMDateUtils.truncate(evaluationDate, Calendar.MONTH, Deployment.getDeployment(tenantID).getTimeZone());
    Date startDay = day;
    int totalDays = Calendar.getInstance().getActualMaximum(Calendar.DAY_OF_MONTH);
    Date endDay = RLMDateUtils.addDays(day, totalDays, Deployment.getDeployment(tenantID).getTimeZone());
    return this.getValue(startDay, endDay);
  }

...
public class ReportEsReader {
    ...
    /**
     * Starts processing. This is a synchronous call : when it returns, all messages have been sent to the Kafka topic.
     * <p>
	...
	public void start() {
		...
		for (int i=0; i<esIndex.length; i++) {
			...
			try {
				...
				SearchRequest searchRequest = new SearchRequest(indexToRead)
				...
				if (searchHits != null) {
					...
					log.debug("getFailedShards = "+searchResponse.getFailedShards());
					log.debug("getSkippedShards = "+searchResponse.getSkippedShards());
					log.debug("getTotalShards = "+searchResponse.getTotalShards());
					log.debug("getTook = "+searchResponse.getTook());
					log.debug("searchHits.length = "+searchHits.length

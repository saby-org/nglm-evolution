  ...
  public boolean evaluate(SubscriberEvaluationRequest evaluationRequest)
  {
    ...
    ****************************************/
    ...
    try
      ...
      {
        log.info("invalid criterion field {}", criterionField.getID());
        StringWriter stackTraceWriter = new StringWriter();
        e.printStackTrace(new PrintWriter(stackTraceWriter, true));
        log.info(stackTraceWriter.toString());
        evaluationRequest.subscriberTrace("TrueCondition : criterionField {0} not supported", criterionField.getID());
        return true;
      }

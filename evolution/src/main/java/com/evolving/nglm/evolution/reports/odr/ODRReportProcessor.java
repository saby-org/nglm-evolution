...
/**
 * This class implements phase 2 for the ODR Report.
 * It implements {@link ReportProcessorFactory} and is passed as parameter when instancing {@link ReportProcessor}.
 *
 */
public class ODRReportProcessor implements ReportProcessorFactory {

    private static final Logger log = LoggerFactory.getLogger(ODRReportProcessor.class);

    final static String STORENAME = "evol-store";
    private String kafkaNode;
    private String topicOut;

    final AtomicInteger nbOdr = new AtomicInteger(0);
    ...
    private KTable<String, ReportElement> odrJoined;
        ...
        odrJoined = groupedBy
            .aggregate(
                ...
                (k, v, agg) -> {
                    ...
                    final int indexOdr = 0;
                    final int indexSubscriber = 1;
                    if (v.type == indexOdr) {
                        Map<String, Object> odrFieldsV = v.fields.get(indexOdr);
                        if (re.fields.get(indexOdr) == null)
                          {
                            re.fields.put(indexOdr, new HashMap<>());
                          }
                        re.fields.get(indexOdr).put(String.valueOf(index.getAndIncrement()), odrFieldsV);
                        if (re.fields.get(indexSubscriber) != null) {
                          // log.trace("------------> complete record 1 ! "+v+" "+re);
                          re.isComplete = true;
                        }
                        nbOdr.incrementAndGet();
    ...
    public void afterStart(final KafkaStreams streams) {
        ...
        do {
            ...
            int nowOdr = nbOdr.get();
            int speedOdr = (nowOdr-beforeOdr)/delay;
            beforeOdr = nowOdr;
        ...
        final String queryableStoreName = odrJoined.queryableStoreName(); // returns null if KTable is not queryable
    ...
    public static void main(String[] args) {
        ...
        for(String arg : args){
          log.info("ODRReportProcessor: arg " + arg);
        }

        if (args.length < 6) {
            log.warn("Usage : ODRReportProcessor topicIn topicOut KafkaNode ZKhostList appIdSuffix instance");
            return;
        }
        ...
        ReportProcessorFactory reportFactory = new ODRReportProcessor();
        ...
        log.info("Finished ODRReportProcessor");
    }

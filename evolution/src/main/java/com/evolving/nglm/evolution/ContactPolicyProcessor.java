        ...
        for (MessageLimits limit : channels.getMessageLimits())
          {
            ...
            if (requestMetricHistory.getValue(RLMDateUtils.truncate(startDate, Calendar.DATE, Deployment.getDeployment(tenantID).getBaseTimeZone()), RLMDateUtils.truncate(evaluationDate, Calendar.DATE, Deployment.getDeployment(tenantID).getBaseTimeZone())) >= limit.getMaxMessages())
              {
                returnValue = true;
                break;
              }

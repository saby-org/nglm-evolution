        ...
        for (MessageLimits limit : channels.getMessageLimits())
          {
            ...
            if (requestMetricHistory.getValue(RLMDateUtils.truncate(startDate, Calendar.DATE, Deployment.getBaseTimeZone()), RLMDateUtils.truncate(evaluationDate, Calendar.DATE, Deployment.getBaseTimeZone())) >= limit.getMaxMessages())
              {
                returnValue = true;
                break;
              }

...
public class OTPUtils {
	...
	static {
		otpTypeService = new OTPTypeService(System.getProperty("broker.servers"), "otpinstance-otptypeservice",
				Deployment.getOTPTypeTopic(), false);
...
//	}
	...
	public static OTPInstanceChangeEvent burnAllOTPsFromProfile(OTPInstanceChangeEvent otpRequest,
		...
		List<OTPInstance> candidates = profile.getOTPInstances().stream()
				.filter(c -> statusListValidToBurn.contains(c.getOTPStatus())).collect(Collectors.toList());
		List<OTPInstance> nonCandidates = profile.getOTPInstances().stream()
				.filter(c -> !statusListValidToBurn.contains(c.getOTPStatus())).collect(Collectors.toList());

		for (OTPInstance updateOtp : profile.getOTPInstances().stream()
				.filter(c -> statusListValidToBurn.contains(c.getOTPStatus())).collect(Collectors.toList())) {
	...
	public static OTPInstanceChangeEvent handleOTPEvent(OTPInstanceChangeEvent otpRequest, SubscriberProfile profile,
			OTPTypeService otpTypeService, int tenantID) {
	...
	public static OTPInstanceChangeEvent burnOTPsFromGivenType(OTPInstanceChangeEvent otpRequest,
			SubscriberProfile profile, OTPTypeService otpTypeService, int tenantID) {
		...
		List<OTPInstance> candidates = profile.getOTPInstances().stream()
				.filter(c -> c.getOTPTypeDisplayName().equals(otpRequest.getOTPTypeName())
						& statusListValidToBurn.contains(c.getOTPStatus()))
	...
	public static OTPInstanceChangeEvent checkOTP(OTPInstanceChangeEvent otpRequest, SubscriberProfile profile,
			OTPTypeService otpTypeService, int tenantID) {
		...
		if (tomatch.getOTPValue().equals(otpRequest.getOTPCheckValue())) {
			...
			int globalerrorstimewindow = candidates.stream()
					.filter(c -> DateUtils.addSeconds(c.getLatestError(), otptype.getTimeWindow()).after(now))
					.mapToInt(o -> o.getErrorCount()).sum();
	...
	public static void clearOldOTPs(SubscriberProfile profile, OTPTypeService otpTypeService, int tenantID) {
		...
		for (OTPInstance oldOtp : otps) {
			...
			if (DateUtils.addSeconds(oldOtp.getCreationDate(), otpType.getTimeWindow()).before(now)
					&& oldOtp.getExpirationDate().before(now)
					&& oldOtp.getOTPStatus() != OTPInstance.OTPStatus.RaisedBan)
				continue;
			// otp banned but bannedperiod outlonged ... can be deleted
			if (oldOtp.getOTPStatus() == OTPInstance.OTPStatus.RaisedBan
					&& DateUtils.addSeconds(oldOtp.getLatestUpdate(), otpType.getBanPeriod()).before(now))
				continue;
			// not in a "terminal" status but passed the expiration, keep for global counts
			// but set it to expired
			if (oldOtp.getOTPStatus() != OTPInstance.OTPStatus.Expired
					&& oldOtp.getOTPStatus() != OTPInstance.OTPStatus.Burnt
					&& oldOtp.getOTPStatus() != OTPInstance.OTPStatus.RaisedBan
					&& oldOtp.getExpirationDate().before(now)) {
	...
	// OTP Creation
	public static OTPInstanceChangeEvent generateOTP(OTPInstanceChangeEvent otpRequest, SubscriberProfile profile,
			OTPTypeService otpTypeService, int tenantID) {
		...
		Date now = new Date();
		List<OTPInstance> initialOtpList = profile.getOTPInstances().stream()
				.filter(c -> c.getOTPTypeDisplayName().equals(otpRequest.getOTPTypeName()))
				.collect(Collectors.toList());
		...
		try {
			...
			// Check 01 : not during a ban issue
			// testing only the latest should be enough
			if (mostRecentOtp.getOTPStatus().equals(OTPInstance.OTPStatus.RaisedBan)
					&& DateUtils.addSeconds(mostRecentOtp.getLatestUpdate(), otptype.getBanPeriod()).after(now)) {
				// maybe add some other return values ??
				otpRequest.setReturnStatus(RESTAPIGenericReturnCodes.CUSTOMER_NOT_ALLOWED);
				return otpRequest;
			}

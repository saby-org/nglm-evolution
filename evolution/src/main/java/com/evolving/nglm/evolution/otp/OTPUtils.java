...
public class OTPUtils
{
  ...
  /**************************************************************
   *
   * Evolution Engine message handling SECTION basically ee calls handleOTPEvent
   * with an OTPInstanceChangeEvent request and expects an OTPInstanceChangeEvent
   * response and depending on the Action this reroutes to either generateOTP :
   * generates a new otp and invalidate any previous otp checkOTP : tries to
   * validate a code on the active otp
   **************************************************************/
  ...
  // no known call yet but meant to force invalidate all otp when a security issue
  // asked to invalidate them
  ...
  private static void markAsError(OTPInstance instanceToError, OTPStatus errorToUseForInstance, boolean allowEscalateInstanceStatus, OTPInstanceChangeEvent requestToError, RESTAPIGenericReturnCodes errorToUseForRequest, boolean allowEscalateRequestStatus, OTPType otptype, List<OTPInstance> sameTypeInstancesList, Date updateDate)
  {
    ...
    if (allowEscalateRequestStatus && remainingAttempts <= 0)
      requestToError.setReturnStatus(RESTAPIGenericReturnCodes.MAX_NB_OF_ATTEMPT_REACHED);
    ...
    if (allowEscalateRequestStatus && globalerrorstimewindow >= otptype.getMaxWrongCheckAttemptsByTimeWindow())
      requestToError.setReturnStatus(RESTAPIGenericReturnCodes.MAX_NB_OF_ATTEMPT_REACHED);
  }
    ...
    try
      {
        ...
        if (profile == null)
          {
            // probably already raised before it reaches this section but for safety reasons lets reject
        ...
        if (tomatch.getOTPStatus().equals(OTPStatus.Burnt))
          {
            // keep burnt but count the error and possibly ban since this could be seen as
            // invalid
            markAsError(tomatch, OTPStatus.Burnt, false, otpRequest, RESTAPIGenericReturnCodes.OTP_BURNT, true, otptype, candidates, now);
            return otpRequest;
          }
        ...
        if (tomatch.getOTPValue().equals(otpRequest.getOTPCheckValue()))
          {
            ...
            if (otpRequest.getForceBurn() != null && otpRequest.getForceBurn())
              tomatch.setOTPStatus(OTPStatus.Burnt);
          } 
        else
          {
            // Mismatch => mark as error with possible escalation
            markAsError(tomatch, OTPStatus.ChecksError, true, otpRequest, RESTAPIGenericReturnCodes.INVALID_OTP, true, otptype, candidates, now);
          }
      } 
    catch (Exception e)
      {
  ...
  // TODO : check this is included in a workflow that updates the profile
  // afterwards
  public static void clearOldOTPs(SubscriberProfile profile, OTPTypeService otpTypeService, int tenantID)
  {
    ...
    // not empty, lets check for elements that are older than the timewindow or
    // banperiod
        ...
        try
          {
            ...
            if (oldOtp.getCreationDate() == null)
              continue;

            // otp outside relevant time windows can be deleted/skipped
            // (and not banned since ban has its own window)
            if (DateUtils.addSeconds(oldOtp.getCreationDate(), otpType.getTimeWindow()).before(now) && oldOtp.getExpirationDate().before(now) && oldOtp.getOTPStatus() != OTPStatus.RaisedBan)
              continue;

            // otp banned but banperiod outlonged ... can be deleted
            if (oldOtp.getOTPStatus() == OTPStatus.RaisedBan && DateUtils.addSeconds(oldOtp.getLatestUpdate(), otpType.getBanPeriod()).before(now))
              continue;
    ...
    try
      {
        ...
        // ( force profile.setOTPInstances since it could have been null pointer and not
        // an existing List )
        if (existingInstances == null)
          existingInstances = new ArrayList<OTPInstance>();
        ...
        for (Pair<DialogTemplate, String> template : templates)
          {
            EvolutionUtilities.sendMessage(evolutionEventContext, tags, template.getFirstElement().getDialogTemplateID(), ContactType.ActionNotification, "OTP", template.getSecondElement(), subscriberEvaluationRequest, subscriberState, otpRequest.getFeatureID(), otpRequest.getModuleID());
          }
  ...
  /**************************************************************
   *
   * TOOLS for instance's otp codes generation (See if this can be mutualized with
   * TokenUtils class ... )
   *
   **************************************************************/

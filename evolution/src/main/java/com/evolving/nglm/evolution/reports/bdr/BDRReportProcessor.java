 ...
 *
 */
public class BDRReportProcessor implements ReportProcessorFactory {
    ...
    final AtomicInteger nbBdr = new AtomicInteger(0);
    ...
    private KTable<String, ReportElement> bdrJoined;
        ...
        bdrJoined = groupedBy
            .aggregate(
                ...
                (k, v, agg) -> {
                    ...
                    final int indexBdr = 0;
                    final int indexSubscriber = 1;
                    if (v.type == indexBdr) {
                        Map<String, Object> bdrFieldsV = v.fields.get(indexBdr);
                        if (re.fields.get(indexBdr) == null)
                          {
                            re.fields.put(indexBdr, new HashMap<>());
                          }
                        re.fields.get(indexBdr).put(String.valueOf(index.getAndIncrement()), bdrFieldsV);
                        if (re.fields.get(indexSubscriber) != null) {
                            // log.trace("------------> complete record 1 ! "+v+" "+re);
                            re.isComplete = true;
                        }
                        nbBdr.incrementAndGet();
    ...
    public void afterStart(final KafkaStreams streams) {
        ...
        do {
            ...
            int nowBdr = nbBdr.get();
            int speedBdr = (nowBdr-beforeBdr)/delay;
            beforeBdr = nowBdr;
        ...
        final String queryableStoreName = bdrJoined.queryableStoreName(); // returns null if KTable is not queryable
    ...
    public static void main(String[] args) {
        ...
        for(String arg : args){
          log.info("BDRReportProcessor: arg " + arg);
        }

        if (args.length < 6) {
            log.warn("Usage : BDRReportProcessor topicIn topicOut KafkaNode ZKhostList appIdSuffix instance");
            return;
        }
        ...
        log.info("Finished BDRReportProcessor");
    }

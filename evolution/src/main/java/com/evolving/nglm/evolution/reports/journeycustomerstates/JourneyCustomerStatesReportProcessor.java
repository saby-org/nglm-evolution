...
/**
 * This class implements phase 2 for the Journey Report.
 * It implements {@link ReportProcessorFactory} and is passed as parameter when instancing {@link ReportProcessor}.
 *
 */
public class JourneyCustomerStatesReportProcessor implements ReportProcessorFactory {

	private static final Logger log = LoggerFactory.getLogger(JourneyCustomerStatesReportProcessor.class);

	final static String STORENAME = "evol-store";
	private String kafkaNode;
	private String topicOut;

    final AtomicInteger nbJourneyStat = new AtomicInteger(0);
	...
	private KTable<String, ReportElement> journeyJoined;
	 ...
	 * <li>
	 * If it contains journey info, we add it in the list of journeys for this customer (we create the list
        ...
        journeyJoined = groupedBy
            .aggregate(
                ...
                (k, v, agg) -> {
                    ...
                    final int journeyStat = 0;
                    final int indexSubscriber = 1;
                    if (v.type == journeyStat) {
                        Map<String, Object> journeyStatV = v.fields.get(journeyStat);
                        if (re.fields.get(journeyStat) == null)
                          {
                            re.fields.put(journeyStat, new HashMap<>());
                          }
                        re.fields.get(journeyStat).put(String.valueOf(index.getAndIncrement()), journeyStatV);
                        if (re.fields.get(indexSubscriber) != null) {
                          // log.trace("------------> complete record 1 ! "+v+" "+re);
                          re.isComplete = true;
                        }
                        nbJourneyStat.incrementAndGet();
	...
	public void afterStart(final KafkaStreams streams) {
      ...
      final int delay = 5;
		do {
          ...
          int nowStat = nbJourneyStat.get();
          int speedStat = (nowStat-beforeStat)/delay;
          beforeStat = nowStat;
		...
		final String queryableStoreName = journeyJoined.queryableStoreName(); // returns null if KTable is not queryable
	...
	public static void main(String[] args) {
	    ...
	    for(String arg : args){
	      log.info("JourneyReportESReader: arg " + arg);
	    }

		if (args.length < 6) {
			log.warn("Usage : JourneyReportProcessor topicIn topicOut KafkaNode ZKhostList appIdSuffix instance");
			return;
		}
		...
		log.info("Finished JourneyReportProcessor");
	}

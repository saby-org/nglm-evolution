...
public class SMPPGenericPlugin implements NotificationInterface
{
  ...
  *****************************************/
  ...
  private SimpleSMSSender[] senderArray;
  ...
  *****************************************/
  ...
  public void init(DeliveryManagerForNotifications notificationManager, JSONObject notificationPluginConfigurationGlobal)
  {
    //
    ...
    this.connectionFailover = JSONUtilities.decodeBoolean(notificationPluginConfigurationGlobal, "connection_failover", Boolean.FALSE);
    ...
    for (int i=0; i<connections.size(); i++)
      {
        JSONObject notificationPluginConfiguration = (JSONObject) connections.get(i);
        ...
        if(smsSenderFactory.getSMSSenders() == null || (smsSenderFactory.getSMSSenders() != null && smsSenderFactory.getSMSSenders().length == 0))
        ...
        else
          {
            ...
            senderArray[i] = smsSenderFactory.getSMSSenders()[0];
          }
        
        JSONObject evaluationCriterionJSON = JSONUtilities.decodeJSONObject(notificationPluginConfiguration, "criteria", false);
  ...
  *****************************************/
  ...
  public void send(INotificationRequest deliveryNotificationRequest)
  {
    ...
    log.info("RAJ K resolvedParameters {}", resolvedParameters);
    ...
    // select a sender, based on throughput or criteria
    
    SimpleSMSSender sender = null;
    ...
    if (useCriteriaMode) {
      for (int i = 0; i<senderArray.length; i++) {
        SimpleSMSSender currentSender = senderArray[i]; 
          ...
          if (profileValue != null) {
            ...
            if (exp instanceof ConstantExpression) {
              ...
              switch (criteria.getCriterionOperator()) {
                case EqualOperator:
                  if (profileValue.equalsIgnoreCase(value)) sender = currentSender;
                  break;
      ...
      // if no sender matches criteria, or if there are errors, take the first SMSC to send 
      if ((sender == null) && (senderArray.length > 0)) {
      ...
      for (int i = 0; i<senderArray.length; i++) {
        if (thruputMin[i] <= rnd && rnd < thruputMax[i]) {
          ...
          log.trace("selects sender # " + i);
          break;
        }
      }
      if (sender == null) {
        sender = senderArray[0]; // defaults to 1st one

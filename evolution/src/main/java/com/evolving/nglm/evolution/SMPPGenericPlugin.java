...
public class SMPPGenericPlugin implements NotificationInterface
{
  ...
  *****************************************/
  ...
  public void send(INotificationRequest deliveryNotificationRequest)
  {
    ...
    Integer SMSExpiration = (Integer) deliveryRequest.getNotificationParameters().get("node.parameter.SMSexpiration");
    if(sender == null)
      {
      ...
      {
        if(sender.sendSMS(deliveryRequest, text, destination, source, receiptExpected != null ? receiptExpected : false, flashSMS != null ? flashSMS : false, SMSExpiration != null ? SMSExpiration : 240))
          {

...
public class Struct {
	...
	// !!!!!!! START OF ADDED !!!!!!!!!!!!!!!!
	// this class is a copy/past from the original one
	// it got the same name to override the 3rd party jar one
	// the only purpose is to change the method in this one : Struct put(Field field, Object value)
	// to comment the validate value line in it : ConnectSchema.validateValue(field.name(), field.schema(), value);
	// this is surely a very bad thing, but seems like the quickest way and this really save cpu during serialization/deserialization
	static{
		System.out.println("org.apache.kafka.connect.data.Struct LOADED FROM nglm evolution project code");
	}
    // !!!!!!! END OF ADDED !!!!!!!!!!!!!!!!
	...
	public Struct put(Field field, Object value) {
		...
		//ConnectSchema.validateValue(field.name(), field.schema(), value); 	// !!!!!!!!!!!!!!!!! THE ONLY CHANGED MADE !!!!!!!!!!!!!!!!!!!!!

...
case "${ENTRYPOINT}" in
  ...
  "guimanager")
    exec kafka-run-class -name guiManager -loggc com.evolving.nglm.evolution.GUIManager $GUIMANAGER_PORT $GUI_FWK_API_SERVER
    ;;
  "reportmanager")
  ...
  "reportscheduler")
    ...
    ;;
  "thirdpartymanager")
    exec kafka-run-class -name thirdPartyManager -loggc com.evolving.nglm.evolution.ThirdPartyManager $KEY $API_PORT $GUI_FWK_API_SERVER $THREADPOOL_SIZE $GUIMANAGER_HOST $GUIMANAGER_PORT
    ;;
  "dnboproxy")
    exec kafka-run-class -name dnboproxy -loggc com.evolving.nglm.evolution.DNBOProxy $KEY $API_PORT $DNBOPROXY_THREADS $GUI_FWK_API_SERVER
    ;;
  "datacubemanager")  
  ...
  "purchasefulfillment")
    ...
    ;;
  "ucgengine")
    exec kafka-run-class -name ucgengine -loggc com.evolving.nglm.evolution.UCGEngine
    ;;
  "evolutionengine")
    exec kafka-run-class -name evolutionengine -loggc com.evolving.nglm.evolution.EvolutionEngine /app/runtime $BROKER_SERVERS $KEY $SUBSCRIBERPROFILE_HOST $SUBSCRIBERPROFILE_PORT $INTERNAL_PORT $KAFKA_REPLICATION_FACTOR $KAFKA_STREAMS_STANDBY_REPLICAS
    ;;
  "extractmanager")

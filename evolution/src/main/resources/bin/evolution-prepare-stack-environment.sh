...
DATACUBEMANAGER_CONFIGURATION=`echo $DATACUBEMANAGER_CONFIGURATION | sed 's/ /\n/g' | uniq`
DATACUBEMANAGER_PROMETHEUS=
DATACUBEMANAGER_ENABLED=false
for TUPLE in $DATACUBEMANAGER_CONFIGURATION
do
   export KEY=`echo $TUPLE | cut -d: -f1`
   export HOST=`echo $TUPLE | cut -d: -f2`
   export MONITORING_PORT=`echo $TUPLE | cut -d: -f3`
   if [ -n "$DATACUBEMANAGER_PROMETHEUS" ]; then
     DATACUBEMANAGER_PROMETHEUS="$DATACUBEMANAGER_PROMETHEUS,'$HOST:$MONITORING_PORT'"
   else
     DATACUBEMANAGER_PROMETHEUS="'$HOST:$MONITORING_PORT'"
   fi
   DATACUBEMANAGER_ENABLED=true
done
export DATACUBEMANAGER_PROMETHEUS
export DATACUBEMANAGER_ENABLED

...
BEGIN
  ...
  IF ('true' = 'ITM_INSTALL' ) then
    ...
    SELECT * FROM (SELECT 'ITM' as `app_key`,'ITM' as `app_name`,'InTelestage Triggers Management' as `app_description`,'itm_components/styles/images/itm.png','ITM_URL_WEB_SERVER_PATH' as `web_link`,'/itm' as `app_path`) AS tmp WHERE NOT EXISTS (SELECT `app_key` FROM `dbframework`.`tbl_apps` WHERE `app_key` = 'ITM') LIMIT 1;

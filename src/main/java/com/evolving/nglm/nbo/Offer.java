...
public class Offer extends GUIManagedObject
{
  ...
  *****************************************/
  ...
  public static Object pack(Object value)
  {
    ...
    struct.put("currency", offer.getCurrency().getSupportedCurrencyName());
    ...
    struct.put("offerType", offer.getOfferType().getOfferTypeName());
    struct.put("productType", offer.getProductType().getProductTypeName());
    struct.put("rewardType", offer.getRewardType().getRewardTypeName());
  ...
  ****************************************/
  ...
  private static List<Object> packSalesChannels(Set<SalesChannel> salesChannels)
  {
    ...
    for (SalesChannel salesChannel : salesChannels)
      {
        result.add(salesChannel.getSalesChannelName());
      }
    return result;
  }
  ...
  *****************************************/
  ...
  public Offer(JSONObject jsonRoot, long epoch, GUIManagedObject existingOfferUnchecked) throws GUIManagerException
  {
    ...
    *****************************************/
    ...
    super(jsonRoot, "offerID", (existingOfferUnchecked != null) ? existingOfferUnchecked.getEpoch() : epoch);
    ...
    *****************************************/
    ...
    this.currency = Deployment.getSupportedCurrencies().get(JSONUtilities.decodeString(jsonRoot, "currency", true));
    this.salesChannels = decodeSalesChannels(JSONUtilities.decodeJSONArray(jsonRoot, "salesChannels", true));
    this.profileCriteria = decodeProfileCriteria(JSONUtilities.decodeJSONArray(jsonRoot, "profileCriteria", true));
    this.offerType = Deployment.getOfferTypes().get(JSONUtilities.decodeString(jsonRoot, "offerType", true));
    this.productType = Deployment.getProductTypes().get(JSONUtilities.decodeString(jsonRoot, "productType", true));
    this.rewardType = Deployment.getRewardTypes().get(JSONUtilities.decodeString(jsonRoot, "rewardType", true));
    ...
    *****************************************/
    ...
    if (this.currency == null) throw new GUIManagerException("unsupported currency", JSONUtilities.decodeString(jsonRoot, "currency", true));
    ...
    if (this.productType == null) throw new GUIManagerException("unsupported productType", JSONUtilities.decodeString(jsonRoot, "productType", true));
    if (this.rewardType == null) throw new GUIManagerException("unsupported rewardType", JSONUtilities.decodeString(jsonRoot, "rewardType", true));
  ...
  *****************************************/
  ...
  private Set<SalesChannel> decodeSalesChannels(JSONArray jsonArray) throws GUIManagerException
  {
    ...
    for (int i=0; i<jsonArray.size(); i++)
      {
        String salesChannelName = (String) jsonArray.get(i);
        SalesChannel salesChannel = Deployment.getSalesChannels().get(salesChannelName);
        if (salesChannel == null) throw new GUIManagerException("unknown salesChannel", salesChannelName);
        salesChannels.add(salesChannel);
      }
    return salesChannels;
  }
    ...
    if (existingOffer != null && existingOffer.getAccepted())
      {
        ...
        epochChanged = epochChanged || ! Objects.equals(getName(), existingOffer.getName());
